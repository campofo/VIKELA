...
10. **Live tracking:** once the panic is delivered, the firmware keeps streaming its GPS position to `POST /api/hardware/location` every `LOCATION_TRACK_INTERVAL_MS` (default 5s), tagged with the `alert_id`, so the app can follow the device in real time. It stops when the panic is **resolved** from the app (the location response returns `keep_tracking:false`), after the optional `LOCATION_TRACK_DURATION_MS` cap, or on device reset/power-off. A dropped ping never stops tracking. Configure via `LOCATION_UPDATE_URL`, `LOCATION_TRACK_ENABLED`, `LOCATION_TRACK_INTERVAL_MS`, and `LOCATION_TRACK_DURATION_MS`.
